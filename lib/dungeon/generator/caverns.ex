defmodule Dungeon.Generator.Caverns do
  @moduledoc """
  Natural cavern generation using hybrid approach:
  ...
  @doc """
  Generate natural cavern dungeon
  """
  def generate do
    # Use hybrid approach for reliable connectivity
    foundation = generate_traditional_foundation()
    {grid, caverns} = transform_rooms_to_caverns(foundation)
    grid = transform_corridors_to_passages(grid, foundation.corridors)
    grid = apply_organic_growth(grid, caverns)
  ...
  @doc """
  Get center coordinates of a cavern chamber
  """
  def center(cavern) do
    if cavern.cells != [] do
      # Calculate centroid of all cells
      {sum_x, sum_y} =
        Enum.reduce(cavern.cells, {0, 0}, fn {x, y}, {acc_x, acc_y} ->
          {acc_x + x, acc_y + y}
        end)

      count = length(cavern.cells)
  ...
  @doc """
  Add cavern labels to the grid
  """
  def add_labels(grid, caverns) do
    Enum.reduce(caverns, grid, fn cavern, acc_grid ->
      {center_x, center_y} = center(cavern)

      # Find a suitable position near the center for the label
      case find_label_position(acc_grid, cavern, center_x, center_y) do
        {x, y} -> Map.put(acc_grid, {x, y}, {:area_label, cavern.number})
        nil -> acc_grid
      end
    end)
  end
  ...
  defp grow_cavern_organically(grid, cavern) do
    # Apply organic blob growth around cavern cells
    seed_count = min(3, div(length(cavern.cells), 8))
    seeds = Enum.take_random(cavern.cells, seed_count)
  ...
  defp find_label_position(grid, cavern, center_x, center_y) do
    # Try positions around the center, preferring floor cells within the cavern
    potential_positions = [
      {center_x, center_y},
      {center_x - 1, center_y},
      {center_x + 1, center_y},
    ...
    Enum.find(potential_positions, fn {x, y} ->
      {x, y} in cavern.cells and Map.get(grid, {x, y}) == :floor
    end)
  end
  ...
  defp generate_traditional_foundation do
    # Generate a traditional dungeon as our foundation
  ...
  defp transform_rooms_to_caverns(traditional_dungeon) do
    # Convert traditional rectangular rooms into organic cavern chambers
    caverns =
      traditional_dungeon.rooms
      |> Enum.with_index(1)
      |> Enum.map(fn {room, index} ->
        # Generate organic shape around the room area
        cells = generate_organic_chamber(room)

        %{cells: cells, number: "A#{index}"}
      end)

    # Start with traditional grid and enhance with organic growth
    grid = enhance_grid_with_organic_shapes(traditional_dungeon.grid, caverns)

    {grid, caverns}
  end

  defp transform_corridors_to_passages(grid, corridors) do
    # Convert straight corridors into wider, more natural passages
    Enum.reduce(corridors, grid, fn corridor, acc_grid ->
      widen_corridor_to_passage(acc_grid, corridor.path)
    end)
  end

  defp apply_organic_growth(grid, caverns) do
    # Apply organic blob growth around the foundation caverns
    Enum.reduce(caverns, grid, fn cavern, acc_grid ->
      grow_cavern_organically(acc_grid, cavern)
    end)
  end

  defp generate_organic_chamber(room) do
  ...
  defp enhance_grid_with_organic_shapes(grid, caverns) do
    # Ensure all cavern cells are floor
    Enum.reduce(caverns, grid, fn cavern, acc_grid ->
      Enum.reduce(cavern.cells, acc_grid, fn {x, y}, inner_grid ->
        Map.put(inner_grid, {x, y}, :floor)
      end)
    end)
  end

  defp widen_corridor_to_passage(grid, corridor_path) do
    # Convert corridor to wider passage
